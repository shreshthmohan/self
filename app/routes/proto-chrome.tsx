 ...
 *
 * Four axes, each a URL search param, so a variant is a link a reader can send:
 *
 *   ?body=karla|public|figtree|instrument|inter|mono   the proportional face
 *   ?bg=plain|sunshine                                 codeuncode's backdrop
 *   ?header=plain|rule|bar                             header weight
 *   ?footer=none|line|full                             footer weight
 *
 * The colours, the fonts, and the prose rules all come from `app/app.css`,
 * which carries the proposed `@theme` block. Reading this page IS reading the
 * token set. Content below is fixed prose at real article length, because the
 * question is how a paragraph reads, not how a heading looks.
 */
import type { Route } from "./+types/proto-chrome";

const BODY = ["karla", "public", "figtree", "instrument", "inter", "mono"];
const BG = ["plain", "sunshine"];
const HEADER = ["plain", "rule", "bar"];
const FOOTER = ["none", "line", "full"];
...
export default function ProtoChrome({ loaderData }: Route.ComponentProps) {
	...
	return (
		<div data-body={v.body} data-bg={v.bg} className="min-h-screen pb-40">
			<Header variant={v.header} />
			...
			<main className="mx-auto max-w-2xl px-4 py-10">
				...
				<article className="mt-6">
					...
					<section className="mt-10">
						...
						<div className="prose mt-3">
							<p>
								Markdown is the stored value. A named <code>&lt;textarea&gt;</code>{" "}
								in a real form is the editor. TipTap loads lazily and takes over
								that same field, so the enhancement is additive and the field
								never stops being the thing the form posts.
							</p>
...
if (before !== after) return keepTextarea(diff(before, after))`}</code>
							...
							<p>
								Byte equality was measured first and rejected. Serialisation pads
								table columns and adds a trailing newline, so it refuses plain
								prose and still cannot tell a lost table from a padded one. See{" "}
								<a href="#cost">what it costs</a> for the numbers.
							</p>
						</div>
					</section>
					...
					<section className="mt-10">
						...
						<div className="prose mt-3">
							<p>
								TipTap is 146 kB gzip against a bare React build. It arrives as a
								lazy chunk, so first paint pays about 4 kB and a read page pays
								nothing at all. The alternative that lost, a full markdown editor
								component, measured 357 kB — the research note that claimed 6 kB
								was wrong by sixty times.
							</p>
					...
					{[
						...
						<li key={title}>
							<h3 className="text-xl">
								<a className="underline decoration-border underline-offset-4" href="/proto-chrome">
									{title}
								</a>
							</h3>
...
function Header({ variant }: { variant: string }) {
	...
	if (variant === "bar") {
		return <header className="border-b border-border bg-surface py-4">{inner}</header>;
	}
	if (variant === "rule") {
		return <header className="border-b border-border py-4">{inner}</header>;
	}
	return <header className="py-6">{inner}</header>;
}
...
function Footer({ variant }: { variant: string }) {
	...
	if (variant === "line") {
		return (
			<footer className="mx-auto mt-16 max-w-2xl border-t border-border px-4 py-8 text-sm text-dim">
				Shreshth Mohan
			</footer>
		);
	}
	return (
		<footer className="mt-16 border-t border-border bg-surface">
			<div className="mx-auto flex max-w-2xl flex-wrap gap-x-6 gap-y-2 px-4 py-8 text-sm text-muted">
...
function Bar({ current }: { current: Record<string, string> }) {
	const link = (axis: string, value: string) => {
		const p = new URLSearchParams({ ...current, [axis]: value });
		return `/proto-chrome?${p}`;
	};
		...
		<div className="fixed inset-x-0 bottom-0 border-t border-border bg-surface-2/95 backdrop-blur">
			<div className="mx-auto max-w-3xl space-y-1.5 px-4 py-3 font-mono text-xs">
				...
				<p className="pt-1 text-dim">
					body face on the wire, latin, roman + italic: {BYTES[current.body]}.
					Fraunces roman 67.4 KB and JetBrains Mono roman 31.3 KB ride on top.
					Today the site ships 152.8 KB of Inter alone.
				</p>
			</div>
		</div>
	);
